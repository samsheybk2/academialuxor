    ...
    async function loadCurso() {
      ...
      if (curso) {
        ...
        setForm({
          ...
          max_intentos: curso.max_intentos?.toString() || "3",
        })
          ...
          const modulosConPreguntas = await Promise.all(
            modulosData.map(async (mod: { id: string; titulo?: string; introduccion?: string; video_url?: string; imagen_portada?: string; duracion?: string; max_intentos?: number }) => {
              ...
              return {
                ...
                maxIntentos: mod.max_intentos?.toString() || "",
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            {user?.rol === "decano" || user?.rol === "developer" ? (
              ...
              <div className="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-900 text-sm">
                {facilitadores.find(f => f.id === form.facilitador_id)?.nombre || user?.nombre || "Cargando..."}
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="p-4 space-y-4 border-t border-gray-100">
                    ...
                    <div className="pt-4 border-t border-gray-100">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-sm font-medium text-gray-700">Recursos de descarga</h4>
                        <button
                          type="button"
                          onClick={() => {
                            setMaterialPdf((prev) => [
                          ...
                          className="px-2.5 py-1 bg-violet-50 text-violet-700 rounded-lg text-xs font-medium hover:bg-violet-100 transition-colors flex items-center gap-1"
                        >
                          <Plus className="w-3 h-3" />
                          Agregar recurso
                        </button>
                      ...
                      {materialPdf.filter((m) => m.modulo_id === modulo.id).length === 0 ? (
                        <p className="text-xs text-gray-400">Sin recursos para este módulo</p>
                      ) : (
                        <div className="space-y-3">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">Recursos Generales</h2>
          <button
            type="button"
            onClick={() => {
              setMaterialPdf((prev) => [
        ...
        {materialPdf.filter((m) => m.modulo_id === null).length === 0 ? (
          <p className="text-sm text-gray-400">Sin recursos generales</p>
        ) : (
          <div className="space-y-3">
