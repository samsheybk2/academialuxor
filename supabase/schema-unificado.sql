...
-- 1.12 CURSOS EN RUTAS (relacion N:N)
...
-- 1.14 UNIDADES ORGANIZACIONALES (Direcciones, Gerencias, Departamentos)
...
-- 1.16 FACILITADOR-CARGOS (asignación de cargos a facilitadores)
...
-- 1.19 ETIQUETAS DE EVENTOS (estudiantes etiquetados por facilitadores)
...
-- 1.22 ACTIVIDAD DIARIO DE USUARIO (para rachas)
...
-- 1.23 PUBLICACIONES (Muro de Noticias)
CREATE TABLE IF NOT EXISTS publicaciones (
  ...
  anclado_hasta TIMESTAMPTZ,
  sucursales_destino TEXT[] DEFAULT '{}',
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- 1.30 INSIGNIAS (Badges parametrizables por developer)
CREATE TABLE IF NOT EXISTS insignias (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  nombre TEXT NOT NULL,
  descripcion TEXT,
  imagen_url TEXT,
  parametro TEXT NOT NULL CHECK (parametro IN ('cursos_creados', 'cursos_aprobados', 'estudiantes_capacitados', 'calificacion_promedio')),
...
-- 1.31 INSIGNIAS ASIGNADAS A FACILITADORES
...
-- 1.32 INSIGNIAS ASIGNADAS A ESTUDIANTES
...
-- 1.35 CARGOS INVITADOS A VIDEO CONFERENCIAS
...
-- 1.36 ASISTENCIA A VIDEO CONFERENCIAS
...
-- ============================================================
-- 2. CST (Captacion y Seleccion de Talento)
-- ============================================================
...
-- 2.3 Historial de cambios de etapa de candidatos
...
-- 2.4 Configuracion del modulo CST
...
-- 2.5 Preguntas de competencias (configuracion de tests)
...
-- 2.6 Respuestas puntuadas para cada pregunta de competencia
...
-- 2.7 Sesiones de test (cuando un candidato realiza el test)
...
-- 2.8 Respuestas del candidato en el test
...
-- Lectura para autenticados (admin CST)
...
-- Lectura anonima para preguntas y respuestas (test publico)
...
-- Sesiones: insercion anonima (candidato publico) y lectura admin
...
-- Respuestas: insercion anonima (candidato publico)
...
-- ============================================================
-- 3. NOM (Nomina y Organizacion de Meta)
-- ============================================================

-- 3.1 ESCALAS SALARIALES
...
-- 4.2 Limpiar imágenes de noticias mayores a 30 días
...
-- 4.3 Contador de modulos en curso
...
-- 4.4 Contador de estudiantes en curso
...
-- 4.5 SECURITY DEFINER: leer rol sin circular en RLS
...
-- 4.6 Auto-crear perfil al confirmar email
...
-- 4.7 Funcion para buscar email por username (login)
...
-- 5.1 updated_at en profiles, cursos, rutas
...
-- 5.3 Auto-crear perfil al registrar usuario
...
-- 6.11 UNIDADES ORGANIZACIONALES
...
CREATE INDEX idx_insignias_curso ON insignias(curso_id);
